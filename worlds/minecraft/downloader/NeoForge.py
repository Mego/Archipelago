...
class SemVer:
    def __init__(self, value: str):
        ...
        elif int(slot_split[0]) >= 26:
            # 26+ syntax without a patch number
            self.major = int(slot_split[0])
            self.minor = int(slot_split[1])
            self.patch = 0
        else:
...
class DownloadNeoForge(StepsStep):
    def __init__(
        self, to: str, eula_popup: Callable, force_version: str, heap: str = "2048M"
    ):
    ...
    def _process_vanilla_index(
        self, context: dict[str, Any], vanilla_versions: VanillaVersions
    ) -> str:
        metadata = next(
            (
                ver
                for ver in vanilla_versions["versions"]
                if ver["id"] == self.mc_version
            ),
            None,
        )
    ...
    def _process_vanilla_metadata(
        self, context: dict[str, Any], vanilla_metadata: VanillaMetadata
    ):
    ...
    def _process_versions(
        self, context: dict[str, Any], neo_versions: NeoVersions
    ) -> str:
        ...
        self.neo_latest = next(
            (
                ver
                for ver in reversed(neo_versions["versions"])
                if force_semver.matches(ver)
            ),
            None,
        )
        ...
        self.windows_run = self.all_run + [
            f"@libraries/net/neoforged/neoforge/{self.neo_latest}/win_args.txt"
        ]
        self.unix_run = self.all_run + [
            f"@libraries/net/neoforged/neoforge/{self.neo_latest}/unix_args.txt"
        ]
        self.system_run = (
            self.windows_run
            if Utils.is_windows
            else (self.unix_run if Utils.is_linux else None)
        )
        ...
        if os.path.exists(self.version_path):
            with open(self.version_path, "r") as f:
                if f.read().strip() == self.neo_latest:
                    self.logger.info(
                        f"NeoForge {self.neo_latest} is already installed, skipping download"
                    )
                    return False

        self.logger.info(f"Downloading NeoForge {self.neo_latest} installer")
        return (
            f"https://maven.neoforged.net/releases/net/neoforged/neoforge/{self.neo_latest}/neoforge-{self.neo_latest}-installer.jar",
            None,
            self.neo_latest,
        )
    ...
    def _process_server(self, context: dict[str, Any], req):
        ...
        return (java_path, "-jar", self.installer_jar, "--installServer"), {
            "cwd": self.root,
    ...
    def _process_cleanup(self, context: dict[str, Any], req):
        ...
        context["neoforge_run"] = (
            bat_file if Utils.is_windows else (sh_file if Utils.is_linux else None)
        )

        if req is False:
            # install is skipped
            return ServerInstallData(
                root_dir=self.root, mods_dir=self.mods, run_args=self.system_run
            )
