...
def has_iron_ingots(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    return state.has("Progressive Tools", player) and state.has(
        "Progressive Resource Crafting", player
    )


def has_copper_ingots(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    return state.has("Progressive Tools", player) and state.has(
        "Progressive Resource Crafting", player
...
def has_diamond_pickaxe(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    return state.has("Progressive Tools", player, 3) and has_iron_ingots(
        world, state, player
    )
...
def has_bottle(world: "MinecraftWorld", state: CollectionState, player: int) -> bool:
    return state.has("Bottles", player) and state.has(
        "Progressive Resource Crafting", player
    )
...
def can_enchant(world: "MinecraftWorld", state: CollectionState, player: int) -> bool:
    return state.has("Enchanting", player) and has_diamond_pickaxe(
        world, state, player
    )  # mine obsidian and lapis


def can_use_anvil(world: "MinecraftWorld", state: CollectionState, player: int) -> bool:
    return (
        state.has("Enchanting", player)
...
def fortress_loot(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:  # blaze rods, wither skulls
    return state.can_reach_region("Nether Fortress", player) and basic_combat(
        world, state, player
    )
...
def can_brew_potions(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    return (
        state.has("Blaze Rods", player)
        and state.has("Brewing", player)
        and has_bottle(world, state, player)
    )


def can_piglin_trade(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    return has_gold_ingots(world, state, player) and (
        state.can_reach_region("The Nether", player)
        or state.can_reach_region("Bastion Remnant", player)
    )


def overworld_villager(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    village_region = (
        state.multiworld.get_region("Village", player).entrances[0].parent_region.name
    )
    if (
        village_region == "The Nether"
    ):  # 2 options: cure zombie villager or build portal in village
        return state.can_reach_location("Zombie Doctor", player) or (
    ...
    return state.can_reach_region("Village", player) or state.can_reach_location(
        "Zombie Doctor", player
    )


def enter_stronghold(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    return (
        state.has("Blaze Rods", player)
        and state.has("Brewing", player)
        and state.has("3 Ender Pearls", player)
    )


# Difficulty-dependent functions
def combat_difficulty(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> Literal["easy", "normal", "hard"]:
    return typing.cast(
        Literal["easy", "normal", "hard"], world.options.combat_difficulty.current_key
    )
...
def can_adventure(world: "MinecraftWorld", state: CollectionState, player: int) -> bool:
    ...
    return (
        ...
        and death_link_check
        and (
            state.has("Progressive Resource Crafting", player)
            or state.has("Campfire", player)
        )
    )
...
def basic_combat(world: "MinecraftWorld", state: CollectionState, player: int) -> bool:
    ...
    return (
        state.has("Progressive Weapons", player)
        and (state.has("Progressive Armor", player) or state.has("Shield", player))
        and has_iron_ingots(world, state, player)
    )
...
def complete_raid(world: "MinecraftWorld", state: CollectionState, player: int) -> bool:
    reach_regions = state.can_reach_region(
        "Village", player
...
def can_kill_wither(
    ...
    normal_kill = (
        state.has("Progressive Weapons", player, 3)
        and state.has("Progressive Armor", player, 2)
    ...
    if combat_difficulty(world, state, player) == "easy":
        ...
        combat_difficulty(world, state, player) == "hard"
    ):  # cheese kill using bedrock ceilings
...
def can_kill_ender_dragon(
    ...
    if combat_difficulty(world, state, player) == "hard":
        return (
            ...
            and state.has(
                "Bed", player
            )  # who needs armor when you can respawn right outside the chamber
        )
    return (
        state.has("Progressive Weapons", player, 2)
        and state.has("Progressive Armor", player)
...
def can_kill_elder_guardian(
    world: "MinecraftWorld", state: CollectionState, player: int
) -> bool:
    if not (state.can_reach_region("Ocean Monument", player)):
        # make sure the player can actually reach an ocean monument
        return False
    # combat difficulty logic largely copied from can_kill_ender_dragon, with slight tweaks
    # e.g. archery is not considered because bows are not useful, and bucket access is considered to clear mining fatigue with milk
    match combat_difficulty(world, state, player):
        case "easy":
            return (
                state.has("Progressive Weapons", player, 3)
                and state.has("Progressive Armor", player, 2)
...
def can_kill_warden(
    ...
    if not (state.can_reach_region("Ancient City", player)):
        # make sure the player can actually reach an ancient city
        # note: an ancient city is specifically required here, because it is possible for deep darks to spawn without any sculk shriekers and thus be incapable of spawning a warden
        return False
    # combat difficulty logic largely copied from can_kill_ender_dragon, with slight tweaks
    # e.g. archery is required for all non-hard difficulties, and armor is mostly irrelevant for hard difficulty (since the warden's ranged attacks ignore armor)
    match combat_difficulty(world, state, player):
        case "easy":
            return (
                state.has("Progressive Weapons", player, 3)
        ...
        case "hard":
            return state.has("Progressive Weapons", player, 2) and state.has(
                "Bed", player
            )


def has_structure_compass(
    world: "MinecraftWorld", state: CollectionState, entrance_name: str, player: int
) -> bool:
    if not world.options.structure_compasses:
        return True
    return state.has(
        f"Structure Compass ({state.multiworld.get_entrance(entrance_name, player).connected_region.name})",
        player,
    )
...
def get_rules_lookup(world, player: int):
    rules_lookup = {
        "entrances": {
            ...
            and (
                state.has("Bucket", player) or state.has("Progressive Tools", player, 3)
            )
            and has_iron_ingots(world, state, player),
        ...
        "locations": {
            ...
            "Elder Guardian": lambda state: can_kill_elder_guardian(
                world, state, player
            ),
            "Warden": lambda state: can_kill_warden(world, state, player),
            "Blaze Rods": lambda state: fortress_loot(world, state, player),
            "Who is Cutting Onions?": lambda state: can_piglin_trade(
                world, state, player
            ),
            ...
            and has_iron_ingots(world, state, player),
            "Isn't It Iron Pick": lambda state: state.has(
                "Progressive Tools", player, 2
            )
            and has_iron_ingots(world, state, player),
            "Local Brewery": lambda state: can_brew_potions(world, state, player),
            "The Next Generation": lambda state: can_respawn_ender_dragon(
                world, state, player
            )
            and can_kill_ender_dragon(world, state, player),
            "Fishy Business": lambda state: state.has("Fishing Rod", player),
            "Hot Tourist Destinations": lambda state: has_structure_compass(
                world, state, "Biome Discovery", player
            ),
            ...
            "How Did We Get Here?": lambda state: (
                can_brew_potions(world, state, player)
                ...
                and state.can_reach_region(
                    "Ocean Monument", player
                )  # Heart of the Sea, Dolphin's Grace, Mining Fatigue
            ...
            and can_enchant(world, state, player),
            "Who's the Pillager Now?": lambda state: craft_crossbow(
                world, state, player
            ),
            ...
            "Ice Bucket Challenge": lambda state: has_diamond_pickaxe(
                world, state, player
            ),
            ...
            and can_use_anvil(world, state, player)
            and can_enchant(world, state, player),
            "Arbalistic": lambda state: (
                craft_crossbow(world, state, player)
                and state.has("Piercing IV Book", player)
                and can_use_anvil(world, state, player)
                and can_enchant(world, state, player)
            ),
            "The End... Again...": lambda state: can_respawn_ender_dragon(
                world, state, player
            )
            and can_kill_ender_dragon(world, state, player),
            "Acquire Hardware": lambda state: has_iron_ingots(world, state, player),
            'Not Quite "Nine" Lives': lambda state: can_piglin_trade(
                world, state, player
            )
            and state.has("Progressive Resource Crafting", player, 2),
            "Cover Me with Diamonds": lambda state: state.has(
                "Progressive Armor", player, 2
            )
            and state.has("Progressive Tools", player, 2)
            and has_iron_ingots(world, state, player),
            "Sky's the Limit": lambda state: basic_combat(world, state, player),
            "Hired Help": lambda state: state.has(
            ...
            "You Need a Mint": lambda state: can_respawn_ender_dragon(
                world, state, player
            )
            and has_bottle(world, state, player),
            "Monsters Hunted": lambda state: can_respawn_ender_dragon(
                world, state, player
            )  # Ghast, Hoglin, Magma Cube, Piglin
            and can_kill_ender_dragon(
                world, state, player
            )  # Ender Dragon, Enderman, Endermite, Silverfish
            and can_kill_wither(
                world, state, player
            )  # Blaze, Wither, Wither Skeleton, Zombified Piglin
            ...
            and (
                (
                    can_brew_potions(world, state, player)
                    and state.has("Fishing Rod", player)
                )  # Water Breathing Potions for Elder Guardian, Guardian
                or (
                    can_enchant(world, state, player) and state.has("Bucket", player)
                )  # Aqua Affinity/Respiration and Milk/Axolotls for Elder Guardian, Guardian
            ),
            ...
            "Serious Dedication": lambda state: (
                state.can_reach_location("Hidden in the Depths", player)
            ...
            and state.has("Bed", player)
            and has_diamond_pickaxe(world, state, player),
            "Beaconator": lambda state: (
                can_kill_wither(world, state, player)
                and has_diamond_pickaxe(world, state, player)
                and state.has("Progressive Resource Crafting", player, 2)
            ),
            "Withering Heights": lambda state: can_kill_wither(world, state, player),
            "A Balanced Diet": lambda state: (
                has_bottle(world, state, player)  # honey bottle
            ...
            "Country Lode, Take Me Home": lambda state: state.has(
                "Progressive Tools", player, 2
            )
            and has_iron_ingots(world, state, player),
            ...
            and has_iron_ingots(world, state, player)
            and state.has("Bucket", player),
            "The Healing Power of Friendship": lambda state: can_adventure(
                world, state, player
            )
            and has_iron_ingots(world, state, player)
            and state.has("Bucket", player),
            ...
            "Whatever Floats Your Goat!": lambda state: can_adventure(
                world, state, player
            ),
            ...
            and (
                ...
                or state.can_reach_region(
                    "Nether Fortress", player
                )  # soul sand in fortress if not in nether for water elevator
            ...
            and has_iron_ingots(world, state, player)
            and state.has("Progressive Tools", player, 2),
            "When the Squad Hops into Town": lambda state: can_adventure(
                world, state, player
            )
            and state.has("Lead", player)
            and state.has("Bucket", player)
            and has_iron_ingots(world, state, player),
            "With Our Powers Combined!": lambda state: can_adventure(
                world, state, player
            )
            ...
            and state.has("Bucket", player)
            and has_iron_ingots(world, state, player),
            "You've Got a Friend in Me": lambda state: state.can_reach_region(
                "Pillager Outpost", player
            )
            or (
                basic_combat(world, state, player)
                and state.can_reach_region("Woodland Mansion", player)
            ...
            "Crafting a New Look": lambda state: has_iron_ingots(
                world, state, player
            )  # Maybe streamline this one
            and (
                fortress_loot(world, state, player)
                or (
                    state.can_reach_region("Pillager Outpost", player)
            ...
            "Smithing with Style": lambda state: can_excavate(
                world, state, player
            ...
            "Careful Restoration": lambda state: state.can_reach_region(
                "Trial Chambers", player
            )
            and basic_combat(world, state, player),
            "The Power of Books": lambda state: state.has(
                "Progressive Tools", player, 2
            ),
            ...
            "Crafters Crafting Crafters": lambda state: has_iron_ingots(
                world, state, player
            )
            and state.has("Progressive Tools", player, 2),
            ...
            "Mob Kabob": lambda state: state.has(
                "Progressive Resource Crafting", player
            ),
...
def set_rules(self: "MinecraftWorld") -> None:
    ...
    def defeated_bosses(state: CollectionState) -> bool:
        return (
            (not bosses.dragon or state.has("Ender Dragon", player))
            and (not bosses.wither or state.has("Wither", player))
            and (not bosses.elder_guardian or state.has("Elder Guardian", player))
            and (not bosses.warden or state.has("Warden", player))
        )

    egg_shards = min(
        self.options.egg_shards_required.value, self.options.egg_shards_available.value
    )
    completion_requirements = lambda state: (
        location_count(state) >= self.options.advancement_goal
        and state.has("Dragon Egg Shard", player, egg_shards)
    )
    multiworld.completion_condition[player] = lambda state: completion_requirements(
        state
    ) and defeated_bosses(state)
